            ...
            <div className="item grow relative flex">
                ...
                <div className="grow grid grid-cols-3 grid-rows-3 gap-2 md:grid-cols-8 md:grid-rows-1 md:gap-0">
                    <div className="col-span-3 grid grid-cols-3 grid-rows-[1fr_min-content_1fr]">
                        ...
                        <div className="item-data flex items-center md:rounded-tl-lg px-2 md:py-1" title={`${buy?.toLocaleString()}z`}>{buy?.toLocaleString()}z</div>
                        <div className="item-data flex items-center px-2 md:py-1 border-l" title={`${sell?.toLocaleString()}z`}>{sell?.toLocaleString()}z</div>
                        <div className="item-data flex items-center px-2 md:py-1 border-l" title={weight?.toLocaleString()}>{weight?.toLocaleString()}</div>
                    </div>
                    <div className="col-span-3 grid grid-cols-3 grid-rows-[1fr_min-content_1fr]">
                        ...
                        <div className="item-data item-separator flex items-center px-2 md:py-1 md:border-l">
                            {((mobDropCount ?? 0) > 0 || (mobMvpDropCount ?? 0) > 0) && <div className="flex items-center">
                                ...
                                <span className="truncate" title={`${((mobDropCount ?? 0) + (mobMvpDropCount ?? 0)).toLocaleString()} drops`}>
                        ...
                        <div className="item-data flex items-center px-2 md:py-1 border-l">
                            {((productSum ?? 0) > 0) && <div className="flex items-center">
                                ...
                                <span className="truncate" title={`x${productSum?.toLocaleString()}`}>x{productSum?.toLocaleString()}</span>
                            </div>}
                        </div>
                        <div className="item-data flex items-center px-2 md:py-1 border-l">
                            {((repeatableProductSum ?? 0) > 0) && <div className="flex items-center">
                                <Icon className="shrink-0 emphasis" name="repeat" />
                                <span className="truncate" title={`x${repeatableProductSum?.toLocaleString()}`}>x{repeatableProductSum?.toLocaleString()}</span>
                            </div>}
                        </div>
                    </div>
                    <div className="col-span-2 grid grid-cols-2  grid-rows-[1fr_min-content_1fr]">
                        ...
                        <div className="item-data item-separator flex rounded-bl-lg md:rounded-none items-center px-2 md:py-1 md:border-l">
                            {((ingredientSum ?? 0) > 0) && <div className="flex items-center">
                                <Icon className="shrink-0 emphasis" name="star" />
                                <span className="truncate" title={`x${ingredientSum?.toLocaleString()}`}>x{ingredientSum?.toLocaleString()}</span>
                            </div>}
                        </div>
                        <div className="item-data flex items-center rounded-tr-lg md:rounded-tr-none md:rounded-br-lg px-2 md:py-1 border-l">
                            {((repeatableIngredientSum ?? 0) > 0) && <div className="flex items-center overflow-hidden">
                                <Icon className="shrink-0 emphasis" name="repeat" />
                                <span className="truncate" title={`x${repeatableIngredientSum?.toLocaleString()}`}>x{repeatableIngredientSum?.toLocaleString()}</span>
                            </div>}
                        </div>
                    </div>
